...
const Publications = () => {
  ...
  const [indexForAbstract, setIndexForAbstract] = useState(null);

  const handleModalOpen = (index) => {
    setIsModalVisible(true);
    setIndexForAbstract(index);
  };

  const handleModalClose = () => {
    setIsModalVisible(false);
    setIndexForAbstract(null);
  };
  ...
  return (
    <div className="mt-4">
      ...
      <div className="p-8 lg:p-20 grid gap-4 lg:grid-cols-3 lg:grid-rows-3 lg:h-[900px] w-full h-auto">
        ...
        {workingPublications.map((pub, index) => (
          <div
            key={index}
            className="p-8 lg:col-span-1 lg:row-span-1 h-[200px] lg:h-auto bento-cell relative group text-[14px] flex flex-col"
      ...
      {isModalVisible && (
        <Modal
          abstract={workingPublications[indexForAbstract]?.abstract} // Pass abstract content
